...
class InfoPage extends React.Component {
    ...
    this.setState({
      ...
      Axios.get(
        `https://api.worldtradingdata.com/api/v1/stock?symbol=${
          this.props.symbol
        }&api_token=webEtV2VViO9zqzRoxhxVCoUtoG8JYkzJyXMwbmxBYG9RcYjMxh9KoKhejAB`
      ).then(
        function(response) {
          this.setState({
  ...
  historicalPrice() {
    ...
    Axios.get(
      `https://api.worldtradingdata.com/api/v1/history?symbol=${
        this.state.infoInput
      }&sort=newest&api_token=webEtV2VViO9zqzRoxhxVCoUtoG8JYkzJyXMwbmxBYG9RcYjMxh9KoKhejAB`
    ).then(
      function(response) {
        console.log(response.data.history[formatDate]);
        if (!response.data.history[formatDate]) {
          //undefined is falsy, so this checks if the value is undefined
  ...
  render() {
    return (
      <div>
        ...
        <DatePicker size="large" onChange={this.dateValue}>
          Historical Lookup
        </DatePicker>
        ...
        <h4>
          <Link to="/">
            <Button type="primary" style={divStyleRight}>
              Go to Homepage
            </Button>
          </Link>
        </h4>
